...
module.exports = async (req, res) => {
  ...
  for (const host of YH_HOSTS) {
    try {
      const r = await fetch(`https://${host}/v8/finance/chart/${encodeURIComponent(ysym)}?interval=1mo&range=max`, { headers: UA });
      if (!r.ok) { lastErr = new Error('HTTP ' + r.status); continue; }
      const d = await r.json();
      const res0 = d && d.chart && d.chart.result && d.chart.result[0];
      const ts = (res0 && res0.timestamp) || [];
      const cl = (res0 && res0.indicators && res0.indicators.quote && res0.indicators.quote[0] && res0.indicators.quote[0].close) || [];
      const pts = [];
      for (let i = 0; i < ts.length; i++) {
        if (cl[i] > 0) pts.push([ts[i], +cl[i].toFixed(4)]);
      }
      // السعر اللحظي كنقطة أخيرة إن كان أحدث
      const m = res0 && res0.meta;
      if (m && m.regularMarketPrice > 0 && pts.length && m.regularMarketTime > pts[pts.length - 1][0]) {
      ...
      return res.status(200).json({ sym, market, count: pts.length, points: pts });
    } catch (e) { lastErr = e; }
  }
  res.setHeader('Cache-Control', 'no-store');
